...
MAX_MODEL_RECIPE_CANDIDATES = 32
...
RECIPE_RECOMMENDATION_SKILL_SPEC = dedent(
    ...
    4. 最多返回 16 个食谱，按适配度从高到低排序；如果服务器食谱库没有合适结果，可以返回空 items，不要补外部或生成内容。
...
def build_recipe_recommendation_messages(payload: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    try:
        requested_limit = int(context.get("recommendation_limit") or MAX_RECOMMENDATION_ITEMS)
    except (TypeError, ValueError):
        requested_limit = MAX_RECOMMENDATION_ITEMS
    recommendation_limit = min(MAX_RECOMMENDATION_ITEMS, max(4, requested_limit))

    user_prompt = (
        f"请只从服务器已有食谱库 payload.recipes 中推荐最多 {recommendation_limit} 个食谱，并按适配度从高到低排序。\n"
        f"服务器食谱候选预览：{recipe_preview}\n"
        "请结合 payload.recent_meals 中最近餐次记录，避免连续推荐过于相似的主食、肉类或口味。\n"
